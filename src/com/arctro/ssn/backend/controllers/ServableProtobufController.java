...
/**
 * A common ancestor for ServabkeProtobufControllers
 * @author Ben McLean
 *
 * @param <S> The ServableProtobufWrapper that this controller controls
 * @param <C> The Collection type of S
 */
public abstract class ServableProtobufController<S extends ServableProtobufWrapper<?>, C extends ProtobufCollectionWrapper<?>> extends ProtobufController<S,C>{
	
	/**
	 * Initializes the ProtobufController
	 * @param context The request's SSNContext
	 */
	public ServableProtobufController(SSNContext context){
		super(context);
	}
